...
const FILTER_CHIPS: { key: string; label: string }[] = [
  { key: 'all', label: 'Tous' },
  { key: 'todo', label: 'À interpréter' },
  { key: 'progress', label: 'En cours' },
  { key: 'done', label: 'Terminés' },
];

/** Champs optionnels si l’API les ajoute sur les lignes ECG (aucun montant affiché). */
type EcgBillingFields = EcgRecordItem & {
  interpretation_billing?: 'premium' | 'free';
  billing_tier?: 'premium' | 'free';
};
...
function payantGratuit(item: EcgRecordItem): 'payant' | 'gratuit' {
  const r = item as EcgBillingFields;
  if (r.interpretation_billing === 'premium' || r.billing_tier === 'premium') return 'payant';
  if (r.interpretation_billing === 'free' || r.billing_tier === 'free') return 'gratuit';
  return 'payant';
}

function BillingKindBadge({ item }: { item: EcgRecordItem }) {
  const isPayant = payantGratuit(item) === 'payant';
  return (
    <View className={isPayant ? 'bg-green-100 px-2 py-0.5 rounded-full' : 'bg-blue-100 px-2 py-0.5 rounded-full'}>
      <Text
        className={isPayant ? 'text-green-800 text-[10px] font-semibold' : 'text-blue-800 text-[10px] font-semibold'}
      >
        {isPayant ? 'Payant' : 'Gratuit'}
      </Text>
    </View>
  );
}
...
export default function CardiologueMesEcgScreen() {
  const { user } = useAuth();
  ...
  const { records, loading, error, refetch } = useMyAssignedEcg(user?.id);

  const filtered = useMemo(() => {
    let list = records;
    if (chip === 'todo') {
      list = list.filter(
        r => r.status === 'pending' || r.status === 'assigned' || r.status === 'validated',
      );
    } else if (chip === 'progress') {
  ...
  const emptyAssigned = !loading && records.length === 0 && !error;
  ...
  return (
    <View className="flex-1 bg-gray-50 dark:bg-zinc-950" style={{ paddingTop: insets.top }}>
      <View className="px-4 pt-3 pb-2 border-b border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-900">
        <Text className="text-xl font-bold text-gray-900 dark:text-zinc-100">Mes ECG assignés</Text>
        <Text className="text-xs text-gray-500 dark:text-zinc-400 mt-0.5">
          ECG qui vous ont été confiés directement
        </Text>
        <TextInput
      ...
      >
        {error ? (
          ...
          <Text className="text-center text-gray-500 dark:text-zinc-400 py-16 text-sm px-2">
            Aucun ECG ne vous a été assigné pour le moment
          </Text>
        ) : filtered.length === 0 ? (
          <Text className="text-center text-gray-500 dark:text-zinc-400 py-16 text-sm">
            Aucun ECG pour ce filtre.
          </Text>
        ) : (
          filtered.map(item => {
                ...
                >
                  <View className="flex-row items-start">
                    ...
                    <View className="flex-1">
                      <View className="flex-row items-center justify-between mb-1 gap-2 flex-wrap">
                        <Text className="text-sm font-semibold text-gray-900 dark:text-zinc-100 flex-1 min-w-[120px]" numberOfLines={1}>
                          {item.patient_name}
                          {isUrgent ? ' ⚡' : ''}
                        </Text>
                        <View className="flex-row items-center gap-1.5 flex-shrink-0">
                          <BillingKindBadge item={item} />
